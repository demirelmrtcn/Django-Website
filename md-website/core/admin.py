...
# ============================================================
# DECISION MAKER ADMIN
# ============================================================

class WheelOptionInline(admin.TabularInline):
    model = WheelOption
    extra = 1
    fields = ['text', 'color', 'order']


@admin.register(DecisionWheel)
class DecisionWheelAdmin(admin.ModelAdmin):
    list_display = ['name', 'user', 'template_badge', 'option_count', 'updated_at']
    list_filter = ['is_template', 'created_at']
    search_fields = ['name']
    inlines = [WheelOptionInline]
    list_select_related = ['user']
    
    @admin.display(description='Sablon', boolean=True)
    def template_badge(self, obj):
        return obj.is_template
    
    @admin.display(description='Secenek Sayisi')
    def option_count(self, obj):
        return obj.options.count()


@admin.register(WheelOption)
class WheelOptionAdmin(admin.ModelAdmin):
    list_display = ['text', 'wheel', 'color', 'order']
    list_filter = ['wheel']
    search_fields = ['text', 'wheel__name']
    list_select_related = ['wheel']


@admin.register(DecisionHistory)
class DecisionHistoryAdmin(admin.ModelAdmin):
    list_display = ['wheel_name', 'result', 'user', 'decided_at']
    list_filter = ['decided_at']
    search_fields = ['wheel_name', 'result']
...
class WatchItemAdmin(admin.ModelAdmin):
    list_display = ['title', 'item_type', 'status', 'rating_display', 'year', 'progress_badge', 'shared_badge']
    list_filter = ['item_type', 'status', 'is_shared', 'created_at']
    ...
    fieldsets = (
        ('Genel Bilgiler', {
            'fields': ('user', 'title', 'item_type', 'status', 'is_shared')
        }),
        ('Metadata', {
            'fields': ('year', 'genre', 'creator', 'poster_url', 'backdrop_url', 'overview', 'tmdb_id'),
            'classes': ('collapse',)
        }),
        ('Ilerleme', {
            'fields': (
        ...
        ('Degerlendirme', {
            'fields': ('rating', 'review', 'favorite_quotes')
        }),
        ('Tarihler', {
    ...
    @admin.display(description='Ilerleme')
    def progress_badge(self, obj):
        progress = obj.progress_percentage
        if progress > 0:
            color = '#28a745' if progress >= 75 else '#ffc107' if progress >= 50 else '#17a2b8'
            return format_html(
                '<div style="background-color: #e9ecef; border-radius: 10px; width: 100px; height: 20px; overflow: hidden;">'
                '<div style="background-color: {}; width: {}%; height: 100%; text-align: center; color: white; font-size: 11px; line-height: 20px;">{:.0f}%</div>'
                '</div>',
                color, progress, progress
            )
        return '-'
    
    @admin.display(description='Puan', ordering='rating')
    def rating_display(self, obj):
        if obj.rating:
            return format_html('*** <strong>{}</strong>', obj.rating)
        return '-'
    
    @admin.display(description='Ortak', boolean=True)
    def shared_badge(self, obj):
        return obj.is_shared
    
    actions = ['mark_as_completed', 'mark_as_planning', 'mark_as_ongoing', 'export_as_csv']
    
    @admin.action(description='OK Tamamlandi olarak isaretle')
    def mark_as_completed(self, request, queryset):
        from django.utils import timezone
        updated = queryset.update(status='completed', completed_at=timezone.now().date())
        self.message_user(request, f'{updated} item tamamlandi olarak isaretlendi.')
    
    @admin.action(description='Izleyecegim olarak isaretle')
    def mark_as_planning(self, request, queryset):
        updated = queryset.update(status='planning')
        self.message_user(request, f'{updated} item izleyecegim olarak isaretlendi.')
    
    @admin.action(description='Izliyorum olarak isaretle')
    def mark_as_ongoing(self, request, queryset):
        updated = queryset.update(status='ongoing')
        self.message_user(request, f'{updated} item izliyorum olarak isaretlendi.')
    
    @admin.action(description='CSV Export')
    def export_as_csv(self, request, queryset):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = 'attachment; filename="watchitems.csv"'
        
        writer = csv.writer(response)
        writer.writerow(['Title', 'Type', 'Status', 'Rating', 'Progress', 'Year', 'Creator'])
...
class PlaceVisitInline(admin.TabularInline):
    model = PlaceVisit
    extra = 0
    fields = ['visit_date', 'rating', 'cost', 'would_return', 'is_favorite']
    readonly_fields = ['visit_date']
    can_delete = False
    
    def get_queryset(self, request):
        qs = super().get_queryset(request)
        return qs.order_by('-visit_date')[:10]  # Only show last 10 visits


@admin.register(Place)
class PlaceAdmin(admin.ModelAdmin):
    list_display = ['name', 'category', 'cuisine_type', 'city', 'district', 'rating_display', 'visit_count', 'avg_cost']
    list_filter = ['category', 'city', 'price_range']
    search_fields = ['name', 'cuisine_type', 'address']
    readonly_fields = ['times_visited', 'average_rating', 'average_cost', 'created_at', 'updated_at']
    list_per_page = 50
    list_select_related = ['user']  # Performance
    
    fieldsets = (
        ('Temel Bilgiler', {
            'fields': ('user', 'name', 'category', 'cuisine_type', 'price_range')
        }),
        ('Konum', {
        ...
        ('Istatistikler', {
            'fields': ('times_visited', 'average_rating', 'average_cost'),
            'classes': ('collapse',)
        }),
        ('Harici Entegrasyonlar', {
            'fields': ('google_place_id', 'photo_url', 'photo_photographer', 'unsplash_id'),
            'classes': ('collapse',)
        }),
    )
    
    inlines = [PlaceVisitInline]
    
    def get_queryset(self, request):
        qs = super().get_queryset(request)
        return qs.annotate(visit_count_cached=Count('visits'))
    
    @admin.display(description='Ortalama Puan', ordering='average_rating')
    def rating_display(self, obj):
        if obj.average_rating > 0:
            stars = '***' * int(obj.average_rating)
            return format_html('<strong>{} {:.1f}</strong>', stars, float(obj.average_rating))
        return '-'
    
    @admin.display(description='Ziyaret', ordering='visit_count_cached')
    def visit_count(self, obj):
        return f'{obj.visit_count_cached}x'
    
    @admin.display(description='Ort. Harcama', ordering='average_cost')
    def avg_cost(self, obj):
        if obj.average_cost > 0:
            return format_html('<strong>{:.0f} TL</strong>', float(obj.average_cost))
        return '-'


@admin.register(PlaceVisit)
class PlaceVisitAdmin(admin.ModelAdmin):
    list_display = ['place', 'user', 'visit_date', 'rating', 'cost', 'favorite_badge']
    list_filter = ['visit_date', 'is_favorite', 'would_return']
    search_fields = ['place__name', 'notes', 'what_we_ate']
    date_hierarchy = 'visit_date'
    readonly_fields = ['created_at']
    list_select_related = ['place', 'user']
    
    @admin.display(description='Favori', boolean=True)
...
class PlaceRecommendationAdmin(admin.ModelAdmin):
    list_display = ['source_place', 'similarity_score', 'created_at']
    list_filter = ['created_at']
    search_fields = ['source_place__name']
    readonly_fields = ['created_at']
    list_select_related = ['source_place']
